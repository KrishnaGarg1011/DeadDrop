...
const selectedExpire = computed(() => {
  if (expire.value === null) return 'Never';
  const h = expire.value;
...
async function submit() {
  error.value = '';
  if (type.value === 'text' && !secretText.value.trim()) {
  ...
  if (type.value === 'text') fd.append('secret_text', secretText.value);
  else fd.append('file', file.value);

  if (expire.value !== null && expire.value !== 'Never') {
    fd.append('expires_in_hours', String(expire.value));
  }
  if (viewLimit.value !== 'unlimited') {
    fd.append('max_views', String(viewLimit.value));
  }
  if (usePassword.value) {
    fd.append('is_password_protected', 'true');
    fd.append('password', password.value);
  }
  if (burnAfterReading.value) fd.append('burn_after_reading', 'true');
...
const features = [
  ...
  { icon: '🔒', title: 'Password protected', desc: 'Optional bcrypt-hashed passwords with automatic lockout.' },
  { icon: '⏱', title: 'Expiry & view limits', desc: 'Time-based access with strict view-count enforcement.' },
  { icon: '📋', title: 'Full audit trail', desc: 'Every access attempt is logged with IP and state changes.' },
];
  ...
  <div class="home">
    ...
    <section class="panel">
      <div v-if="createdPkg" class="card created">
        <h2>Drop created 🎉</h2>
        <p class="muted">Send this link to your recipient.</p>
        <div class="link-box">
...
.link-box { display: flex; gap: 10px; margin: 16px 0; }
